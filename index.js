...
function logFactory(formatJSON = true) {
  ...
  function logItem(top = false) {
    return content => {
      ...
      logContainer.insertAdjacentHTML(top ? `afterbegin` : `beforeend`,`<li${isHead ? ` class="head"` : ``}><div>${content}</div></li>` );
    };
  }
